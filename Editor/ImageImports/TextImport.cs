    ...
    public class TextImport : IImageImport
    {
        public void DrawImage(PSImage image, GameObject parent, GameObject ownObj = null)
        {
            ...
            if (ownObj != null)
                ...
                myText = PSDImportUtility.LoadAndInstant<UnityEngine.UI.Text>(PSD2UGUIConfig.ASSET_PATH_TEXT, image.name, parent);
            ...
            {
                fontFolder = PSD2UGUIConfig.FONT_STATIC_FOLDER;
            }
            else
            {
                fontFolder = PSD2UGUIConfig.FONT_FOLDER;
            }
            string fontFullName = fontFolder + image.arguments[1] + PSD2UGUIConfig.FONT_SUFIX;
            ...
            {
                Debug.LogError("outline: " + image.outline);
                ...
                if (_temp.Length > 2)
                {
                    Vector2 effectDistance = Vector2.one;
                    if (int.TryParse(_temp[1], out int x))
                    {
                        effectDistance.x = x;
                    }

                    if (int.TryParse(_temp[2], out int y))
                    {
                        effectDistance.y = y;
                    }
