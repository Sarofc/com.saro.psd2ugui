...
{
    [CustomLayer(ELayerType.TabGroup)]
    /// <summary>
    /// 自定义页签组件的生成
    /// </summary>
    public class TabGroupLayerImport : ILayerImport
    {
        ...
        {
            TabGroup tabGroup = PSDImportUtility.LoadAndInstant<TabGroup>(PSD2UGUIConfig.ASSET_PATH_TABGROUP, layer.name, parent);
        ...
        private Tab ImportTabLayer(Layer layer, GameObject parent)
        {
            ...
            Tab tab = PSDImportUtility.LoadAndInstant<Tab>(PSD2UGUIConfig.ASSET_PATH_TAB, tabName, parent);
            tab.tag = tag;
